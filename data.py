...
Outputs: model.joblib, model_metrics.json
"""
...
def main():
    ...
    fi_data = {fname: float(imp) for fname, imp in zip(feature_names, importances)}
    for k, v in sorted(fi_data.items(), key=lambda x: x[1], reverse=True):
        print(f"  {k:15s}: {v:.4f}")
